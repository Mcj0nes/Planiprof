-- Fix: recréer les policies RLS des causeries avec EXISTS (évite les conflits RLS imbriqués)

-- Tables (idempotent si la migration précédente a quand même créé les tables)
CREATE TABLE IF NOT EXISTS causeries_grids (
...
CREATE TABLE IF NOT EXISTS causeries_students (
  id         uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS causeries_scores (
...
-- Supprimer les policies existantes (peu importe leur état)
DROP POLICY IF EXISTS "users manage own causeries grids"    ON causeries_grids;
DROP POLICY IF EXISTS "users manage own causeries students" ON causeries_students;
DROP POLICY IF EXISTS "users manage own causeries scores"   ON causeries_scores;

-- Recréer avec EXISTS (même pattern que le carnet de notes)
