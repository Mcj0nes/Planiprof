...
async function autoImportFromGradebooks(
  ...
  const { data: gbBooks } = await supabase
    .from('grade_books').select('id').eq('user_id', userId)
  ...
  const { data: gbStudents } = await supabase
    ...
    .in('grade_book_id', gbIds)
    .order('sort_order').order('name')
  if (!gbStudents?.length) return

  const { data: existing } = await supabase
    .from('causeries_students').select('name').eq('grid_id', gridId)
  const existingNames = new Set((existing ?? []).map((s: any) => s.name.toLowerCase().trim()))

  const { data: lastRow } = await supabase
    .from('causeries_students').select('sort_order').eq('grid_id', gridId)
  ...
  if (rows.length > 0) {
    await supabase.from('causeries_students').insert(rows)
  }
}
...
export async function loadGrid(gridNumber = 1, etape: number | null = null): Promise<{
  ...
  let query = supabase.from('causeries_grids').select('id').eq('user_id', user.id)
  ...
  if (grids.length === 0) {
    const { data: g } = await supabase
      .from('causeries_grids').insert(insertPayload).select('id').single()
    if (!g) return null
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('causeries_scores')
      .select('student_id, criterion, score')
...
export async function loadEtapeOverview(etape: number): Promise<{
  ...
  const [{ data: allStudents }, { data: jugData }] = await Promise.all([
    supabase
      .from('causeries_students')
      .select('id, name, sort_order, grid_id')
      .in('grid_id', gridIds)
      .order('sort_order'),
    supabase
      .from('observation_jugements')
      .select('student_name, score')
      .eq('user_id', user.id)
      .eq('grid_type', 'Causeries')
      .eq('etape', etape),
  ])
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      ...
      .in('student_id', studentIds)
      .eq('criterion', 'global')
    globalScores = (data ?? []) as { student_id: string; score: number }[]
  }
...
export async function loadAllGridsForPrint(etape: number): Promise<{
  grids: {
    gridNumber: number
    students: { name: string; scores: Record<string, number | null> }[]
  }[]
}> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  ...
  const { data: allGrids } = await supabase
    .from('causeries_grids').select('id')
  ...
  const { data: allStudents } = await supabase
    .from('causeries_students').select('id, name, sort_order, grid_id')
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('causeries_scores').select('student_id, criterion, score').in('student_id', studentIds)
...
export async function setEtapeJugement(etape: number, studentName: string, score: number | null): Promise<void> {
  ...
  if (score === null) {
    ...
    await supabase.from('observation_jugements')
      .upsert(
        { user_id: user.id, grid_type: 'Causeries', etape, student_name: studentName, score },
...
export async function deleteGrid(gridId: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('causeries_grids').delete().eq('id', gridId).eq('user_id', user.id)
  revalidatePath('/dashboard/evaluation/observation/causeries-mathematiques')
}
...
export async function syncStudentsFromGradebooks(gridId: string): Promise<void> {
  ...
  const { data: grid } = await supabase
    .from('causeries_grids').select('id').eq('id', gridId).eq('user_id', user.id).single()
  if (!grid) return
...
export async function addStudent(gridId: string, name: string): Promise<void> {
  ...
  await supabase.from('causeries_students').insert({ grid_id: gridId, name, sort_order: nextOrder })
  revalidatePath('/dashboard/evaluation/observation/causeries-mathematiques')
}
...
export async function updateStudentName(studentId: string, name: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('causeries_students').update({ name }).eq('id', studentId)
}
...
export async function removeStudent(studentId: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('causeries_students').delete().eq('id', studentId)
  revalidatePath('/dashboard/evaluation/observation/causeries-mathematiques')
}
...
export async function setScore(studentId: string, criterion: string, score: number | null): Promise<void> {
  ...
  if (score === null) {
    await supabase.from('causeries_scores').delete().eq('student_id', studentId).eq('criterion', criterion)
  } else {
    await supabase.from('causeries_scores').upsert({ student_id: studentId, criterion, score })
  }
}
...
export async function resetAllScores(gridId: string): Promise<void> {
  ...
  const { data: students } = await supabase
    .from('causeries_students').select('id').eq('grid_id', gridId)

  if (students?.length) {
    await supabase.from('causeries_scores').delete()
