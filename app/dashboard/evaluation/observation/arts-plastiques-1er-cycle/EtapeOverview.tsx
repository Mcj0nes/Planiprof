...
export default function EtapeOverview({ etape, grids, rows }: Props) {
  ...
  const printAllUrl = `/dashboard/evaluation/observation/arts-plastiques-1er-cycle/print-all?etape=${etape}`
        ...
        {([1, 2, 3] as const).map(n => (
          <button key={n} onClick={() => router.push(toUrl(n, undefined, 'ensemble'))}
            className={`text-xs px-3 py-1.5 rounded-full font-medium border transition ${
              etape === n ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300'
            }`}
          >
            Étape {n}
          </button>
        ))}
        <button onClick={() => router.push('?grille=1')} className="text-xs text-gray-400 hover:text-gray-600 underline transition ml-1">
          Toutes les grilles
        </button>
      </div>
