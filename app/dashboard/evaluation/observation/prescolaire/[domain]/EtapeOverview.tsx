...
const LEVEL_LABELS: Record<number, string> = {
  1: 'DA — Début d\'apprentissage',
  2: 'EA — En apprentissage',
  3: 'EC — En consolidation',
  4: 'A — Acquis',
}

const SCORE_CODES = ['DA', 'EA', 'EC', 'A']
...
function ScoreColor({ score, separator }: { score: number | null; separator?: boolean }) {
  const rounded = score !== null ? Math.round(score) as 1 | 2 | 3 | 4 : null
  const c = rounded !== null ? LEVEL_COLORS[rounded] : null
  const code = rounded !== null ? SCORE_CODES[rounded - 1] : null
  return (
    <td
      className={`border border-gray-200 text-center align-middle text-xs font-semibold
        ${c ? `${c.bg} ${c.border}` : 'text-gray-300'}`}
...
function JugementCell({ domain, etape, studentName, initialScore }: {
  domain:       string
  etape:        number
  studentName:  string
  ...
  const code = score !== null ? SCORE_CODES[score - 1] : null
  ...
  function commit(raw: string) {
    ...
    setEditing(false)
    setInput('')
    startTransition(() => setEtapeJugement(domain, etape, studentName, next))
  }
  ...
  return (
    <td
      className={`border border-gray-200 text-center align-middle cursor-pointer select-none transition-colors text-xs font-semibold
        ${c ? `${c.bg} ${c.border}` : 'bg-white hover:bg-gray-50'}`}
...
export default function EtapeOverview({ domain, etape, grids, rows }: Props) {
  const router = useRouter()
  const [showPrintMenu, setShowPrintMenu] = useState(false)

  const printAllUrl = `/dashboard/evaluation/observation/prescolaire/${domain}/print-all?etape=${etape}`
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        ...
        {([1, 2, 3] as const).map(n => (
          <button
            key={n}
            onClick={() => router.push(toUrl(n, undefined, 'ensemble'))}
      ...
      <div className="flex items-center gap-1.5 mb-5 flex-wrap">
        {grids.map(g => (
          <button
            key={g.id}
            onClick={() => router.push(toUrl(etape, g.gridNumber))}
        ...
        <button className="text-xs px-3 py-1.5 rounded-full font-medium border bg-blue-600 text-white border-blue-600 cursor-default">
          Vue d&apos;ensemble
        </button>
      </div>
            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  style={{ width: 90, borderLeft: '2px solid #c7d2fe' }}
                >
                ...
                <th
                  ...
                  style={{ width: 110, borderLeft: '2px solid #a5b4fc' }}
                >
              ...
              {rows.map((row, idx) => {
                ...
                return (
                  <tr key={row.name} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'}>
                    <td className="border-b border-r border-gray-200 px-3 py-2 sticky left-0 z-10 bg-inherit text-sm text-gray-800">
                      {row.name}
                    </td>
                    {row.scores.map((score, i) => (
                      <ScoreColor key={i} score={score} />
                    ))}
          ...
          {([1, 2, 3, 4] as const).map(n => {
            ...
            return (
              <div key={n} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${c.bg} ${c.border}`}>
                <span className={`w-8 h-6 flex items-center justify-center rounded-md text-xs font-bold border ${c.border} bg-white/70`}>
                  {SCORE_CODES[n - 1]}
                </span>
