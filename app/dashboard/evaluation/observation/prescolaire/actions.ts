...
function pathFor(domain: string) {
  return `/dashboard/evaluation/observation/prescolaire/${domain}`
}
...
async function autoImportFromGradebooks(
  ...
  const { data: existing } = await supabase
    .from('prescolaire_obs_students').select('name').eq('grid_id', gridId)
  const existingNames = new Set((existing ?? []).map((s: any) => s.name.toLowerCase().trim()))

  const { data: lastRow } = await supabase
    .from('prescolaire_obs_students').select('sort_order').eq('grid_id', gridId)
  ...
  if (rows.length > 0) await supabase.from('prescolaire_obs_students').insert(rows)
}

export async function loadGrid(domain: string, gridNumber = 1, etape: number | null = null): Promise<{
  gridId: string
  gridNumber: number
  totalGrids: number
  etape: number | null
  ...
  let query = supabase.from('prescolaire_obs_grids').select('id')
  ...
  const insertPayload: Record<string, unknown> = { user_id: user.id, domain }
  if (etape !== null) insertPayload.etape = etape

  if (grids.length === 0) {
    const { data: g } = await supabase.from('prescolaire_obs_grids').insert(insertPayload).select('id').single()
    if (!g) return null
  ...
  const { data: students } = await supabase
    .from('prescolaire_obs_students').select('id, name, sort_order')
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('prescolaire_obs_scores').select('student_id, criterion, score')
...
export async function loadEtapeOverview(domain: string, etape: number): Promise<{
  ...
  const { data: allGrids } = await supabase
    .from('prescolaire_obs_grids').select('id')
    .eq('user_id', user.id).eq('domain', domain).eq('etape', etape)
    .order('created_at', { ascending: true })
  if (!allGrids?.length) return { grids: [], rows: [] }

  const gridIds = allGrids.map(g => g.id)
  const gridType = `Préscolaire-${domain}`

  const [{ data: allStudents }, { data: jugData }] = await Promise.all([
    supabase.from('prescolaire_obs_students').select('id, name, sort_order, grid_id')
      .in('grid_id', gridIds).order('sort_order'),
    supabase.from('observation_jugements').select('student_name, score')
      .eq('user_id', user.id).eq('grid_type', gridType).eq('etape', etape),
  ])
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('prescolaire_obs_scores').select('student_id, score')
...
export async function loadAllGridsForPrint(domain: string, etape: number): Promise<{
  ...
  const { data: allStudents } = await supabase
    .from('prescolaire_obs_students').select('id, name, sort_order, grid_id')
    .in('grid_id', gridIds).order('sort_order')
...
export async function setEtapeJugement(domain: string, etape: number, studentName: string, score: number | null): Promise<void> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  ...
  if (score === null) {
    await supabase.from('observation_jugements').delete()
      .eq('user_id', user.id).eq('grid_type', gridType).eq('etape', etape).eq('student_name', studentName)
  } else {
    await supabase.from('observation_jugements')
      .upsert(
        { user_id: user.id, grid_type: gridType, etape, student_name: studentName, score },
        { onConflict: 'user_id,grid_type,etape,student_name' }
      )
  }
}

export async function addStudent(domain: string, gridId: string, name: string): Promise<void> {
  const supabase = await createClient()
  const { data: existing } = await supabase
    .from('prescolaire_obs_students').select('sort_order').eq('grid_id', gridId)
    .order('sort_order', { ascending: false }).limit(1)
  const nextOrder = ((existing?.[0]?.sort_order) ?? 0) + 1
  await supabase.from('prescolaire_obs_students').insert({ grid_id: gridId, name, sort_order: nextOrder })
  revalidatePath(pathFor(domain))
}

export async function updateStudentName(studentId: string, name: string): Promise<void> {
  const supabase = await createClient()
  await supabase.from('prescolaire_obs_students').update({ name }).eq('id', studentId)
}

export async function removeStudent(domain: string, studentId: string): Promise<void> {
  const supabase = await createClient()
  await supabase.from('prescolaire_obs_students').delete().eq('id', studentId)
  revalidatePath(pathFor(domain))
}
...
export async function setScore(studentId: string, criterion: string, score: number | null): Promise<void> {
  const supabase = await createClient()
  if (score === null) {
    await supabase.from('prescolaire_obs_scores').delete()
      .eq('student_id', studentId).eq('criterion', criterion)
  } else {
    await supabase.from('prescolaire_obs_scores').upsert({ student_id: studentId, criterion, score })
  }
}

export async function resetAllScores(domain: string, gridId: string): Promise<void> {
  const supabase = await createClient()
  const { data: students } = await supabase
    .from('prescolaire_obs_students').select('id').eq('grid_id', gridId)
  if (students?.length) {
    await supabase.from('prescolaire_obs_scores').delete()
...
export async function deleteGrid(domain: string, gridId: string): Promise<void> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('prescolaire_obs_grids').delete().eq('id', gridId).eq('user_id', user.id)
  revalidatePath(pathFor(domain))
}

export async function syncStudentsFromGradebooks(domain: string, gridId: string): Promise<void> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  const { data: grid } = await supabase
    .from('prescolaire_obs_grids').select('id').eq('id', gridId).eq('user_id', user.id).single()
  if (!grid) return
