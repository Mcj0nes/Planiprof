...
function ScoreColor({ score, separator }: { score: number | null; separator?: boolean }) {
  ...
  return (
    <td className={`border border-gray-200 text-center align-middle text-sm font-semibold ${c ? `${c.bg} ${c.border}` : 'text-gray-300'}`}
...
function JugementCell({ etape, studentName, initialScore }: { etape: number; studentName: string; initialScore: number | null }) {
  ...
  function commit(raw: string) {
    ...
    setLocalScore(next); setEditing(false); setInput('')
  ...
  return (
    <td className={`border border-gray-200 text-center align-middle cursor-pointer select-none transition-colors ${c ? `${c.bg} ${c.border}` : 'bg-white hover:bg-gray-50'}`}
...
export default function EtapeOverview({ etape, grids, rows }: Props) {
  ...
  const printAllUrl = `/dashboard/evaluation/observation/sciences-2e-cycle/print-all?etape=${etape}`
