...
import SciencesGrid from './SciencesGrid'
import EtapeOverview from './EtapeOverview'

export default async function Sciences2eCyclePage({
  searchParams,
}: {
  ...
  const nav = (
    <nav className="px-6 py-4 flex items-center gap-3 flex-wrap" style={{ backgroundColor: 'var(--color-nav)' }}>
      ...
      <span className="text-white/40">/</span>
      <h1 className="text-sm font-semibold text-white">Démarche scientifique</h1>
    </nav>
  )

  const header = (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-gray-800">Démarche scientifique</h2>
      <p className="text-sm text-gray-500 mt-1">
        Sciences et technologie — 2e cycle du primaire (3e et 4e année) — Grille d&apos;observation — Niveaux de maîtrise
      </p>
      {vue !== 'ensemble' && (
      ...
      <main className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-8 py-10">
          ...
          <div className="hidden print:block mb-4">
            <h2 className="text-xl font-bold text-gray-900">Démarche scientifique — Vue d&apos;ensemble</h2>
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-8 py-10">
        {header}
        <SciencesGrid key={data.gridId} gridId={data.gridId} gridNumber={data.gridNumber}
          totalGrids={data.totalGrids} etape={data.etape} students={data.students} scores={data.scores} />
      </div>
    </main>
  )
}
