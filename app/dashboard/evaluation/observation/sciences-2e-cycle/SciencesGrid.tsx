...
interface Props {
  gridId: string; gridNumber: number; totalGrids: number; etape: number | null
  students: Student[]; scores: ScoreMap
}

function ScoreCell({ studentId, criterion, initialScore, separator }: {
  studentId: string; criterion: string; initialScore?: number; separator?: boolean
}) {
  ...
  function commit(raw: string) {
    ...
    setLocalScore(next ?? undefined); setInput(''); setEditing(false)
  ...
  return (
    <td
      ...
      style={{ width: separator ? 88 : 72, height: 44, ...(separator ? { borderLeft: '2px solid #a5b4fc' } : {}) }}
...
export default function SciencesGrid({ gridId, gridNumber, totalGrids, etape, students: initial, scores: initialScores }: Props) {
  ...
  async function handleDelete() {
    setShowDelete(false); await deleteGrid(gridId)
  ...
  return (
    <div>
      ...
      <div className="hidden print:block mb-4">
        <h2 className="text-lg font-bold">Démarche scientifique</h2>
      ...
      <div className="print:hidden flex items-center gap-2 mb-6 flex-wrap">
        ...
        {etape !== null && (
          <button onClick={() => router.push(toUrl(null, 1))} className="text-xs text-gray-400 hover:text-gray-600 underline transition ml-1">
            Toutes les grilles
          </button>
        )}
      </div>
                ...
                <td className="border-b border-r border-gray-200 px-3 py-1 sticky left-0 z-10 bg-inherit">
                  {editingId === student.id ? (
                    <input autoFocus value={editName} onChange={e => setEditName(e.target.value)}
                      onBlur={() => commitEditName(student.id)} onKeyDown={e => e.key === 'Enter' && commitEditName(student.id)}
                      className="w-full text-sm border border-blue-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400" />
                  ) : (
                    <span className="text-sm text-gray-800 cursor-text hover:text-blue-600"
...
const LEVEL_DESCRIPTIONS: Record<number, string> = {
  1: "L'élève a besoin d'un soutien constant pour s'engager dans la démarche scientifique.",
  2: "L'élève progresse avec l'accompagnement de l'enseignant ou des pairs.",
  3: "L'élève maîtrise le critère de façon autonome.",
  4: "L'élève dépasse les attentes et enrichit la démarche du groupe.",
}

const CRITERIA_FULL = [
  { key: 'hypothese',     label: 'Question / Hypothèse', description: "Propose une idée simple à vérifier et explique brièvement pourquoi il pense cela." },
  { key: 'manipulation',  label: 'Manipulation',          description: "Utilise le matériel correctement, suit les étapes données et respecte les règles de sécurité." },
  { key: 'observation',   label: 'Observation',           description: "Observe attentivement et décrit seulement ce qu'il voit (mots simples, dessins, tableaux)." },
  { key: 'analyse',       label: 'Analyse',               description: "Fait un lien simple entre ce qu'il a observé et son idée de départ (ex. « quand…, alors… »)." },
  { key: 'conclusion',    label: 'Conclusion',            description: "Explique ce qu'il a compris du phénomène et dit si son idée de départ tenait ou non." },
  { key: 'communication', label: 'Communication',         description: "Présente ses idées clairement avec un vocabulaire scientifique de base (oral, écrit ou schéma)." },
  { key: 'collaboration', label: 'Collaboration',         description: "Participe à la tâche, partage le matériel et écoute les idées des autres." },
]
