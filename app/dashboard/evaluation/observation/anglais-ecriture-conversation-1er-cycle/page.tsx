...
import AnglaisEcritureConvGrid from './AnglaisEcritureConvGrid'
import EtapeOverview from './EtapeOverview'

export default async function AnglaisEcritureConversation1erCyclePage({
  searchParams,
}: {
  ...
  const nav = (
    <nav className="px-6 py-4 flex items-center gap-3 flex-wrap" style={{ backgroundColor: 'var(--color-nav)' }}>
      ...
      <span className="text-white/40">/</span>
      <h1 className="text-sm font-semibold text-white">English — Writing — Conversation — Cycle 1</h1>
    </nav>
  )

  const header = (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-gray-800">English — Writing — Oral Conversation</h2>
      <p className="text-sm text-gray-500 mt-1">
        Cycle 1 — Elementary (Grade 1 &amp; 2) — Observation Grid — Mastery Levels
      </p>
      {vue !== 'ensemble' && (
        <p className="text-sm text-gray-700 font-medium mt-2">
          Click on a cell and type 1, 2, 3 or 4 to record the level. Click on a student&apos;s name to edit it.
        </p>
      )}
    </div>
  )
      ...
      <main className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-8 py-10">
          ...
          <div className="hidden print:block mb-4">
            <h2 className="text-xl font-bold text-gray-900">English — Writing — Oral Conversation — Cycle 1 — Overview</h2>
