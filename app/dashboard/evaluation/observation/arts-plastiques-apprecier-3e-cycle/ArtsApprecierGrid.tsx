...
const CRITERIA = [
  ...
  { key: 'analyse',      label: 'Langage plastique' },
...
function ScoreCell({
  ...
  return (
    ...
    >
      {editing && (
        <input autoFocus value={input} onChange={() => {}} onKeyDown={handleKeyDown}
...
export default function ArtsApprecierGrid({
  ...
  function handleAddStudent() {
    ...
    startTransition(async () => {
      ...
      setStudents(prev => [...prev, { id: crypto.randomUUID(), name, sort_order: (prev[prev.length - 1]?.sort_order ?? 0) + 1 }])
    })
  }
  ...
  function startEditName(student: Student) { setEditingId(student.id); setEditName(student.name) }
  ...
  function handleReset() { setShowReset(false); resetAllScores(gridId).then(() => window.location.reload()) }

  async function handleSync() { setIsSyncing(true); await syncStudentsFromGradebooks(gridId); window.location.reload() }
    ...
    <div>
      <style>{`
        @media print {
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          .print-table-wrap th, .print-table-wrap td { padding: 3px 4px !important; }
        }
      `}</style>

      <div className="hidden print:block mb-4">
        <h2 className="text-lg font-bold">Arts plastiques — Apprécier — 3e cycle</h2>
        <p className="text-sm text-gray-600">{etape ? `Étape ${etape} — ` : ''}Grille {gridNumber}</p>
      </div>
      ...
      <div className="print:hidden flex items-center gap-2 mb-6 flex-wrap">
        ...
        {([1, 2, 3] as const).map(n => (
          <button key={n} onClick={() => router.push(toUrl(n, 1))}
        ...
        {etape !== null && (
          <button onClick={() => router.push(toUrl(null, 1))} className="text-xs text-gray-400 hover:text-gray-600 underline transition ml-1">Toutes les grilles</button>
        )}
      </div>
      ...
      {(totalGrids > 1 || etape !== null) && (
        <div className="print:hidden flex items-center gap-1.5 mb-5 flex-wrap">
          {Array.from({ length: totalGrids }, (_, i) => i + 1).map(n => (
            <button key={n} onClick={() => router.push(toUrl(etape, n))}
              className={`text-xs px-3 py-1.5 rounded-full font-medium border transition ${n === gridNumber ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200 hover:border-indigo-300'}`}>
              Grille {n}
            </button>
          ))}
          {etape !== null && (
            <button onClick={() => router.push(`?etape=${etape}&vue=ensemble`)}
      ...
      <div className="print:hidden flex items-center gap-2 mb-5 flex-wrap">
        <input type="text" value={newName} onChange={e => setNewName(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleAddStudent()} placeholder="Nom de l'élève..."
          className="text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 w-64" />
        <button onClick={handleAddStudent} disabled={isPending || !newName.trim()}
          className="text-sm px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition">Ajouter</button>
        ...
        <div className="ml-auto flex items-center gap-4">
          {!showReset && !showDelete ? (
            <>
              <button onClick={() => setShowReset(true)} className="text-xs text-gray-400 hover:text-red-500 transition">Réinitialiser les résultats</button>
              <button onClick={() => setShowDelete(true)} className="text-xs text-gray-400 hover:text-red-500 transition">Supprimer la grille</button>
            </>
          ) : showDelete ? (
          ...
          <button onClick={() => router.push(toUrl(etape, gridNumber + 1))}
            className="text-xs text-indigo-500 hover:text-indigo-700 font-medium transition whitespace-nowrap">Grille suivante →</button>
        </div>
      </div>
        ...
        <table className="border-collapse text-sm" style={{ minWidth: 730 }}>
          <thead>
            <tr>
              ...
              <th className="p-3 bg-indigo-50 border-b border-r border-gray-200 font-semibold text-indigo-700 text-center"
          ...
          <tbody>
            {students.length === 0 && (
              <tr><td colSpan={CRITERIA.length + 3} className="p-6 text-center text-gray-400 text-sm">Ajoutez des élèves pour commencer.</td></tr>
            )}
              ...
              <tr key={student.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'}>
                <td className="border-b border-r border-gray-200 px-3 py-1 sticky left-0 z-10 bg-inherit">
                  {editingId === student.id ? (
                    ...
                    <span className="text-sm text-gray-800 cursor-text hover:text-blue-600" onClick={() => startEditName(student)}>
                      {student.name || <span className="text-gray-300 italic">Sans nom</span>}
                    </span>
                  )}
                </td>
                {CRITERIA.map(c => (
                  <ScoreCell key={c.key} studentId={student.id} criterion={c.key} initialScore={scores[student.id]?.[c.key]} />
                ))}
                <ScoreCell studentId={student.id} criterion="global" initialScore={scores[student.id]?.['global']} separator />
                <td className="print:hidden border-b border-gray-200 px-2 text-center">
                  <button onClick={() => handleRemoveStudent(student.id)} className="text-gray-300 hover:text-red-400 text-base leading-none transition">✕</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          {([1, 2, 3, 4] as const).map(n => {
            ...
            return (
              <div key={n} className={`flex items-start gap-3 p-3 rounded-xl border ${c.bg} ${c.border}`}>
                <span className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold border ${c.border} bg-white/70`}>{n}</span>
                <div>
...
const LEVEL_DESCRIPTIONS: Record<number, string> = {
  1: "L'élève a besoin d'un soutien constant pour analyser et porter un jugement sur une œuvre.",
  2: "L'élève analyse et apprécie une œuvre avec l'accompagnement de l'enseignant.",
  3: "L'élève apprécie une œuvre de façon autonome en mobilisant le vocabulaire et les concepts plastiques.",
  4: "L'élève formule un jugement esthétique critique, nuancé et bien argumenté, ancré dans un contexte culturel.",
}

const CRITERIA_FULL = [
  { key: 'observation', label: 'Observation de l\'œuvre',      description: "Observe l'œuvre de façon analytique, repère des détails significatifs et des relations entre les éléments." },
  { key: 'description', label: 'Description des éléments',     description: "Décrit avec précision les éléments formels et expressifs de l'œuvre en utilisant le vocabulaire plastique." },
  { key: 'analyse',     label: 'Langage plastique',            description: "Analyse les éléments du langage plastique et explique comment ils contribuent au message ou à l'effet de l'œuvre." },
  { key: 'reaction',    label: 'Réaction personnelle',         description: "Exprime une réaction personnelle nuancée et articulée, en reliant son ressenti à des éléments observés." },
  { key: 'contexte',    label: 'Mise en contexte',             description: "Situe l'œuvre dans son contexte historique, culturel ou artistique et fait des liens avec d'autres œuvres." },
  { key: 'jugement',    label: 'Jugement esthétique',          description: "Formule un jugement esthétique critique, argumenté et fondé sur des observations et une mise en contexte." },
]
