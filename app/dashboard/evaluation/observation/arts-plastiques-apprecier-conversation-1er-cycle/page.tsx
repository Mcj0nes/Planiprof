...
import ArtsApprecierConvGrid from './ArtsApprecierConvGrid'
import EtapeOverview from './EtapeOverview'

export default async function ArtsPlastiquesApprecierConversation1erCyclePage({
  searchParams,
}: {
  ...
  const nav = (
    <nav className="px-6 py-4 flex items-center gap-3 flex-wrap" style={{ backgroundColor: 'var(--color-nav)' }}>
      ...
      <span className="text-white/40">/</span>
      <h1 className="text-sm font-semibold text-white">Arts plastiques — Apprécier — Conversation orale — 1er cycle</h1>
    </nav>
  )

  const header = (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-gray-800">Arts plastiques — Apprécier — Conversation orale</h2>
      <p className="text-sm text-gray-500 mt-1">
        1er cycle du primaire (1re et 2e année) — Grille d&apos;observation — Niveaux de maîtrise
      </p>
      {vue !== 'ensemble' && (
      ...
      <main className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-8 py-10">
          ...
          <div className="hidden print:block mb-4">
            <h2 className="text-xl font-bold text-gray-900">Arts plastiques — Apprécier — Conversation orale — 1er cycle — Vue d&apos;ensemble</h2>
