...
const CRITERIA = [
  { key: 'rythme',       label: 'Rythme / Tempo' },
  { key: 'justesse',     label: 'Justesse' },
  { key: 'technique',    label: 'Technique' },
  { key: 'nuances',      label: 'Nuances' },
  { key: 'phrase',       label: 'Phrasé musical' },
  { key: 'expression',   label: 'Expression / Intention' },
...
export default function MusiqueGrid({
  ...
  return (
    <div>
      ...
      <div className="hidden print:block mb-4">
        <h2 className="text-lg font-bold">Musique — Interpréter — 3e cycle</h2>
      ...
      <div className="print:hidden flex items-center gap-2 mb-5 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <button onClick={() => router.push(toUrl(etape, gridNumber + 1))} className="text-xs text-indigo-500 hover:text-indigo-700 font-medium transition whitespace-nowrap">
            Grille suivante →
          </button>
        </div>
      </div>
        ...
        <table className="border-collapse text-sm" style={{ minWidth: 800 }}>
          <thead>
            <tr>
              ...
              {CRITERIA.map(c => (
                <th key={c.key} className="p-3 bg-gray-100 border-b border-r border-gray-200 font-semibold text-gray-700 text-center" style={{ width: 80 }}>
                  <span className="block text-xs leading-tight">{c.label}</span>
                </th>
              ))}
              <th className="p-3 bg-indigo-50 border-b border-r border-gray-200 font-semibold text-indigo-700 text-center" style={{ width: 88, borderLeft: '2px solid #a5b4fc' }}>
...
const LEVEL_DESCRIPTIONS: Record<number, string> = {
  1: "L'élève a besoin d'un soutien constant pour réaliser l'interprétation.",
  2: "L'élève progresse avec l'accompagnement de l'enseignant.",
  3: "L'élève interprète la pièce de façon autonome et maîtrisée.",
  4: "L'élève dépasse les attentes et fait preuve d'une grande maîtrise expressive.",
}

const CRITERIA_FULL = [
  { key: 'rythme',       label: 'Rythme / Tempo',          description: 'Maintient le tempo et respecte les valeurs rythmiques, y compris les figures complexes.' },
  { key: 'justesse',     label: 'Justesse',                 description: 'Chante ou joue avec une intonation précise et contrôlée tout au long de la pièce.' },
  { key: 'technique',    label: 'Technique',                description: 'Maîtrise la technique vocale ou instrumentale adaptée aux exigences de la pièce.' },
  { key: 'nuances',      label: 'Nuances',                  description: 'Réalise et différencie les nuances avec précision et musicalité.' },
  { key: 'phrase',       label: 'Phrasé musical',           description: 'Structure l\'interprétation selon les phrases musicales et respecte les respirations.' },
  { key: 'expression',   label: 'Expression / Intention',   description: 'Donne un sens musical à l\'interprétation et communique une intention artistique.' },
  { key: 'memorisation', label: 'Mémorisation',             description: 'Interprète avec confiance et sans support, témoignant d\'une appropriation complète de la pièce.' },
]
