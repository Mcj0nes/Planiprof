...
const PATH = '/dashboard/evaluation/observation/musique-apprecier-conversation-2e-cycle'
...
async function autoImportFromGradebooks(
  ...
  const { data: existing } = await supabase
    .from('mus2aconv_obs_students').select('name').eq('grid_id', gridId)
  const existingNames = new Set((existing ?? []).map((s: any) => s.name.toLowerCase().trim()))

  const { data: lastRow } = await supabase
    .from('mus2aconv_obs_students').select('sort_order').eq('grid_id', gridId)
  ...
  if (rows.length > 0) await supabase.from('mus2aconv_obs_students').insert(rows)
}
...
export async function loadGrid(gridNumber = 1, etape: number | null = null): Promise<{
  ...
  let query = supabase.from('mus2aconv_obs_grids').select('id').eq('user_id', user.id)
  ...
  if (grids.length === 0) {
    const { data: g } = await supabase.from('mus2aconv_obs_grids').insert(insertPayload).select('id').single()
    if (!g) return null
  ...
  const { data: students } = await supabase
    .from('mus2aconv_obs_students').select('id, name, sort_order')
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('mus2aconv_obs_scores').select('student_id, criterion, score').in('student_id', studentIds)
    scoreRows = data ?? []
  }
...
export async function loadEtapeOverview(etape: number): Promise<{
  ...
  const { data: allGrids } = await supabase
    .from('mus2aconv_obs_grids').select('id')
  ...
  const [{ data: allStudents }, { data: jugData }] = await Promise.all([
    supabase.from('mus2aconv_obs_students').select('id, name, sort_order, grid_id')
      .in('grid_id', gridIds).order('sort_order'),
    supabase.from('observation_jugements').select('student_name, score')
      .eq('user_id', user.id).eq('grid_type', 'Musique Apprécier Conversation 2e cycle').eq('etape', etape),
  ])
  ...
  if (studentIds.length > 0) {
    const { data } = await supabase
      .from('mus2aconv_obs_scores').select('student_id, score')
...
export async function loadAllGridsForPrint(etape: number): Promise<{
  ...
  const { data: allStudents } = await supabase
    .from('mus2aconv_obs_students').select('id, name, sort_order, grid_id')
    .in('grid_id', gridIds).order('sort_order')
...
export async function setEtapeJugement(etape: number, studentName: string, score: number | null): Promise<void> {
  ...
  if (score === null) {
    await supabase.from('observation_jugements').delete()
      .eq('user_id', user.id).eq('grid_type', 'Musique Apprécier Conversation 2e cycle').eq('etape', etape).eq('student_name', studentName)
  } else {
    await supabase.from('observation_jugements')
      .upsert(
        { user_id: user.id, grid_type: 'Musique Apprécier Conversation 2e cycle', etape, student_name: studentName, score },
...
export async function addStudent(gridId: string, name: string): Promise<void> {
  ...
  await supabase.from('mus2aconv_obs_students').insert({ grid_id: gridId, name, sort_order: nextOrder })
  revalidatePath(PATH)
}

export async function updateStudentName(studentId: string, name: string): Promise<void> {
  const supabase = await createClient()
  await supabase.from('mus2aconv_obs_students').update({ name }).eq('id', studentId)
}

export async function removeStudent(studentId: string): Promise<void> {
  const supabase = await createClient()
  await supabase.from('mus2aconv_obs_students').delete().eq('id', studentId)
  revalidatePath(PATH)
}
...
export async function setScore(studentId: string, criterion: string, score: number | null): Promise<void> {
  const supabase = await createClient()
  if (score === null) {
    await supabase.from('mus2aconv_obs_scores').delete().eq('student_id', studentId).eq('criterion', criterion)
  } else {
    await supabase.from('mus2aconv_obs_scores').upsert({ student_id: studentId, criterion, score })
  }
}

export async function resetAllScores(gridId: string): Promise<void> {
  const supabase = await createClient()
  const { data: students } = await supabase.from('mus2aconv_obs_students').select('id').eq('grid_id', gridId)
  if (students?.length) {
    await supabase.from('mus2aconv_obs_scores').delete().in('student_id', students.map(s => s.id))
  }
  revalidatePath(PATH)
}
...
export async function deleteGrid(gridId: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  await supabase.from('mus2aconv_obs_grids').delete().eq('id', gridId).eq('user_id', user.id)
  revalidatePath(PATH)
}
...
export async function syncStudentsFromGradebooks(gridId: string): Promise<void> {
  ...
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return
  const { data: grid } = await supabase.from('mus2aconv_obs_grids').select('id').eq('id', gridId).eq('user_id', user.id).single()
  if (!grid) return
