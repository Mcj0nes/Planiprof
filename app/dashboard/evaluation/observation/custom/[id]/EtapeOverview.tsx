...
function ScoreColor({ score, separator }: { score: number | null; separator?: boolean }) {
  ...
  return (
    <td
      className={`border border-gray-200 text-center align-middle text-sm font-semibold
...
function JugementCell({ definitionId, etape, studentName, initialScore }: {
  definitionId: string
  etape:        number
  studentName:  string
  ...
  function commit(raw: string) {
    ...
    setEditing(false)
    setInput('')
    startTransition(() => setEtapeJugement(definitionId, etape, studentName, next))
  }
...
export default function EtapeOverview({ definitionId, etape, grids, rows }: Props) {
  const router = useRouter()
  const [showPrintMenu, setShowPrintMenu] = useState(false)

  const printAllUrl = `/dashboard/evaluation/observation/custom/${definitionId}/print-all?etape=${etape}`
              ...
              {rows.map((row, idx) => {
                ...
                return (
                  <tr key={row.name} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/60'}>
                    ...
                    <JugementCell definitionId={definitionId} etape={etape} studentName={row.name} initialScore={row.jugement} />
                  </tr>
                )
              })}
            </tbody>
