...
import { loadDefinition, loadAllGridsForPrint } from '../actions'
...
export default async function CustomObsPrintAllPage({
  params,
  searchParams,
}: {
  ...
  if (!etape) redirect(`/dashboard/evaluation/observation/custom/${definitionId}`)

  const definition = await loadDefinition(definitionId)
  if (!definition) redirect('/dashboard/evaluation/observation')

  const allCriteria = [...definition.criteria, { key: 'global', label: 'Résultat global', description: '' }]
  const data = await loadAllGridsForPrint(definitionId, etape, definition.criteria)
  ...
  return (
    <>
      ...
      <div className="p-8 bg-white min-h-screen">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">{definition.title}</h1>
