...
import type { Criterion } from './actions'
import {
  addStudent, removeStudent, setScore, resetAllScores,
  updateStudentName, syncStudentsFromGradebooks, deleteSession,
} from './actions'
...
function ScoreCell({
  studentId, criterionKey, initialScore, separator,
}: {
  studentId:     string
  criterionKey:  string
  initialScore?: number
  ...
  function commit(raw: string) {
    ...
    setInput('')
    setEditing(false)
    startTransition(() => setScore(studentId, criterionKey, next))
  }
...
export default function CustomGrid({
  definitionId, definitionTitle, criteria,
  sessionId, gridNumber, totalGrids, etape,
  ...
  function handleAddStudent() {
    ...
    startTransition(async () => {
      await addStudent(definitionId, sessionId, name)
  ...
  function handleRemoveStudent(id: string) {
    ...
    startTransition(() => removeStudent(definitionId, id))
  }
  ...
  function handleReset() {
    setShowReset(false)
    resetAllScores(definitionId, sessionId).then(() => window.location.reload())
  }

  async function handleSync() {
    setIsSyncing(true)
    await syncStudentsFromGradebooks(definitionId, sessionId)
    window.location.reload()
  }

  async function handleDelete() {
    setShowDelete(false)
    await deleteSession(definitionId, sessionId)
  ...
  return (
    <div>
      ...
      <div className="hidden print:block mb-4">
        <h2 className="text-lg font-bold">{definitionTitle}</h2>
      ...
      {criteria.some(c => c.description) && (
        <div className="print:hidden bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-6">
      ...
      <div className="print:hidden bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {([1, 2, 3, 4] as const).map(n => {
            ...
            return (
              <div key={n} className={`flex items-center gap-3 p-3 rounded-xl border ${c.bg} ${c.border}`}>
                ...
                <span className="text-sm font-semibold text-gray-800">Niveau {n} — {c.label}</span>
              </div>
            )
          })}
        </div>
