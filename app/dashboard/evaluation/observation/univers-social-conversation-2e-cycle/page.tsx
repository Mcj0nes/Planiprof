...
import USConvGrid from './USConvGrid'
import EtapeOverview from './EtapeOverview'

export default async function USConversation2eCyclePage({
  searchParams,
}: {
  ...
  const nav = (
    <nav className="px-6 py-4 flex items-center gap-3 flex-wrap" style={{ backgroundColor: 'var(--color-nav)' }}>
      ...
      <span className="text-white/40">/</span>
      <h1 className="text-sm font-semibold text-white">Univers social — Oral — 2e cycle</h1>
    </nav>
  )

  const header = (
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-gray-800">Univers social — Conversation orale</h2>
      <p className="text-sm text-gray-500 mt-1">
        2e cycle du primaire (3e et 4e année) — Grille d&apos;observation — Niveaux 1 à 5
      </p>
      {vue !== 'ensemble' && (
        <p className="text-sm text-gray-700 font-medium mt-2">
          Cliquez sur une cellule et tapez 1 à 5 pour enregistrer le niveau. Cliquez sur le nom d&apos;un élève pour le modifier.
        </p>
      )}
    </div>
  )
      ...
      <main className="min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-8 py-10">
          ...
          <div className="hidden print:block mb-4">
            <h2 className="text-xl font-bold text-gray-900">Univers social — Conversation orale — 2e cycle — Vue d&apos;ensemble</h2>
