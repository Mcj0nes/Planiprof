...
import { assignToWeekInTheme, removeFromWeekInTheme, saveWeekNoteInTheme } from './actions'
...
function getWeeksForTheme(startDate: string, endDate: string) {
...
type ThemeAssignment = { id: string; theme_id: string | null; week_start: string | null; content_item_id: number }
...
export default function ThemeGrid({ planId, themeId, startDate, endDate, contentItems, themeAssignments, weekNotes, planContentActivities = [] }: Props) {
  ...
  function handleNoteChange(weekStart: string, field: 'special_activities' | 'reflective_review', value: string) {
    ...
    debounceTimers.current[key] = setTimeout(() => {
      startTransition(async () => { await saveWeekNoteInTheme(planId, weekStart, field, value) })
    }, 800)
  }

  const weeks = getWeeksForTheme(startDate, endDate)

  const weekAssignedIds = new Set(localAssignments.filter(a => a.week_start).map(a => a.content_item_id))
  const themeAssignedIds = new Set(localAssignments.map(a => a.content_item_id))
  const sidebarItems = contentItems.filter(i => themeAssignedIds.has(i.id) && !weekAssignedIds.has(i.id))

  const progress = themeAssignedIds.size ? (weekAssignedIds.size / themeAssignedIds.size) * 100 : 0
  ...
  function handleAssignToWeek(weekStart: string) {
    ...
    if (existing) {
      ...
      setLocalAssignments(prev => [...prev, { id: tempId, theme_id: themeId, week_start: weekStart, content_item_id: selected.id }])
    }
    const captured = selected
    setSelected(null)
    startTransition(async () => { await assignToWeekInTheme(planId, captured.id, themeId, weekStart) })
  }

  function handleRemoveFromWeek(assignment: ThemeAssignment) {
    ...
    startTransition(async () => { await removeFromWeekInTheme(assignment.id, planId, themeId) })
  }
      ...
      <aside className="w-72 shrink-0 bg-white border-r flex flex-col">
        <div className="px-5 py-4 border-b">
          <div className="flex justify-between items-baseline mb-2">
            ...
            <p className="text-xs text-gray-400 tabular-nums">{weekAssignedIds.size} / {themeAssignedIds.size}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {sidebarItems.length === 0 && themeAssignedIds.size > 0 && (
            <div className="flex flex-col items-center justify-center py-10 px-6 text-center">
              <div className="text-3xl mb-2">🎉</div>
              <p className="text-sm font-semibold text-gray-700">Tout est planifié!</p>
              <p className="text-xs text-gray-400 mt-1">Tous les contenus de ce thème ont été assignés à une semaine.</p>
            </div>
          )}

          {themeAssignedIds.size === 0 && (
            <div className="flex flex-col items-center justify-center py-10 px-6 text-center">
              <p className="text-sm font-semibold text-gray-700">Aucun contenu dans ce thème</p>
              <p className="text-xs text-gray-400 mt-1">Assignez des contenus à ce thème dans la planification globale d&apos;abord.</p>
            </div>
          )}
        </div>
      </aside>
                    ...
                    return (
                      <div key={act.id} className="flex items-center gap-2">
                        <a href={`/dashboard/activities/present/${act.id}`} target="_blank" rel="noopener noreferrer"
                          ...
                          <p className="text-sm font-semibold text-gray-800 truncate group-hover:text-indigo-600 flex-1">{act.title}</p>
                        </a>
                        <button
                    ...
                    return (
                      ...
                      >
                        <div className="px-3 py-2.5 flex items-start justify-between" style={{ backgroundColor: bg }}>
                          ...
                          {!isAssignMode && (
                            <Link
                              href={`/dashboard/annual/${planId}/week/${weekStart}?theme=${themeId}`}
