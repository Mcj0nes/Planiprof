...
const GRADE_TO_CYCLE: Record<string, string> = {
  '1re année': 'c1p', '2e année': 'c1p',
  '3e année': 'c2p', '4e année': 'c2p',
  '5e année': 'c3p', '6e année': 'c3p',
  '1re secondaire': 'c1s', '2e secondaire': 'c1s',
  '3e secondaire': 'c2s', '4e secondaire': 'c2s', '5e secondaire': 'c2s',
}

export type NouveauCycleItems = { cycleLabel: string; color: string; bg: string; border: string; nouveau: string[]; retire: string[] }

export function getNouveauItemsForGrade(gradeLabelFr: string): NouveauCycleItems | null {
  const cycleId = GRADE_TO_CYCLE[gradeLabelFr]
  if (!cycleId) return null
  const cycle = CYCLES.find(c => c.id === cycleId)
  if (!cycle) return null
  return { cycleLabel: cycle.label, color: cycle.color, bg: cycle.bg, border: cycle.border, nouveau: cycle.nouveau, retire: cycle.retire }
}
